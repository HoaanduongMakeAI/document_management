frappe.ui.form.on('Folder', {
    refresh: function(frm) {
        ...
        function update_sharepoint_contents_display() {
            ...
            frm.set_html('sharepoint_contents_html', `<div class="text-muted"><i class="fa fa-spinner fa-spin"></i> ${__('Loading contents for:')} <strong>${display_path}</strong></div>`);
            ...
            frappe.call({
                ...
                callback: function(r) {
                    ...
                    if (r.message && r.message.items) {
                        ...
                        frm.set_html('sharepoint_contents_html', html);
                        ...
                        frm.get_field('sharepoint_contents_html').$wrapper.find('a.sharepoint-item').on('click', function(e) {
                            ...
                            if (is_folder) {
                                ...
                                // File actions
                                frappe.confirm(
                                    __("Choose an action for file '{0}':", [item_name]),
                                    [
                                        {
                                            label: __('Open File Link'),
                                            handler: function() {
                                                // Assuming item.webUrl is available in the response for files
                                                // If not, you might need another server call to get the link
                                                if (item.webUrl) {
                                                    window.open(item.webUrl, '_blank');
                                                } else {
                                                    frappe.msgprint(__('File link not available.'));
                                                }
                                            }
                                        },
                                        {
                                            ...
                                            handler: function() {
                                                frappe.new_doc('Incoming Document', {
                                                    ...
                                                    path: new_relative_path_in_folder, // Set the SharePoint path
                        ...
                        frm.set_html('sharepoint_contents_html', html + `<div class="text-danger mt-2">${__('Error')}: ${r.message.error}</div>`);
                    } else if (r.exc) {
                        frm.set_html('sharepoint_contents_html', html + `<div class="text-danger mt-2">${__('Server error while listing contents.')}</div>`);
                        console.error("List Contents Error:", r.exc);
                    } else {
                        frm.set_html('sharepoint_contents_html', html + '<div class="text-muted mt-2">' + __('Folder is empty or unable to load contents.') + '</div>');
                    }
                },
                error: function(err) {
                    frm.set_html('sharepoint_contents_html', html + `<div class="text-danger mt-2">${__('Network error while listing contents.')}</div>`);
